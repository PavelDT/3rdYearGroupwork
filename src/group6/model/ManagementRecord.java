...
public class ManagementRecord {
	...
	/**
	 * Overloaded Constructor - empty
	 */
	public ManagementRecord() {
	}

	/**
	 * Overloaded constructor that takes string parameter
	 * 
	 * @param flightCode
	 */
	...
	/**
	 * Sets the string status based on the numerical status
	 * 
	 * @return
	 */
	public String getStringStatus() {
		...
		if (status == 3)
			return "GROUND CLEARENCE GRANTED";
		if (status == 4)
			return "LANDING";
		if (status == 5)
			return "LANDED";
		...
		if (status == 7)
			return "UNLOADING";
		if (status == 8)
			return "READY CLEAN AND MAINT";
		if (status == 9)
			return "FAULTY AWAIT CLEAN";
		if (status == 10)
			return "CLEAN AWAIT MAINT";
		if (status == 11)
			return "OK AWAIT CLEAN";
		if (status == 12)
			return "AWAIT REPAIR";
		if (status == 13)
			return "READY REFUEL";
		if (status == 14)
			return "READY PASSENGERS";
		if (status == 15)
			return "READY DEPART";
		if (status == 16)
	...
	/**
	 * Sets each status back to a numerical value
	 * 
	 * @param status
	 * @return
	 */
	...
	// NOTE: it's not in the SAAMS Class Diagram diagram
