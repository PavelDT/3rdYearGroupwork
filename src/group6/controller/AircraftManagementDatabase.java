...
public class AircraftManagementDatabase extends Observable {
	 ...
	 * function. This implements singleton.
	 */
	...
	/**
	 * Checks if singleton instance is instantiated. If not it instantiates it. This
	 * implements lazyness and makes sure the same object is used over and over.
	 * 
	 * @return
	 */
	...
	public boolean radarDetect(FlightDescriptor fd) {
		...
		for (int i = 0; i < managementRecords.length; i++) {
			if (managementRecords[i].getStatus() == ManagementRecord.FREE) {
				...
				// setting random status
				int ran = RNG.generateRandomStatus();
				if (ran >= 80) {
					managementRecords[i].setStatus(ManagementRecord.IN_TRANSIT);
				} else {
